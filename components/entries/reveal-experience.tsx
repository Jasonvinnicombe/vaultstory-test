...
export function RevealExperience(props: RevealExperienceProps) {
  ...
  useEffect(() => {
    ...
    const shouldLockScroll = isFullscreen || selectedPhotoIndex !== null;
    if (!shouldLockScroll) {
      return;
    }
  ...
  const contentPanel = (
    <motion.div initial={{ opacity: 0, y: 18 }} animate={{ opacity: 1, y: 0 }} className={`space-y-6 sm:space-y-7 ${isFullscreen ? "mx-auto w-full max-w-6xl py-8 sm:py-10" : ""}`}>
    ...
    <>
      {!isFullscreen ? (
        <div className="space-y-6 sm:space-y-7">
          {screen !== "content" ? introPanel : contentPanel}
        </div>
      ) : null}
          ...
          >
            ...
            <div className="h-full overflow-y-auto p-3 sm:p-5 lg:p-6">
              <div className="mx-auto flex min-h-full max-w-6xl items-center justify-center">
                {screen === "content" ? contentPanel : <div className="w-full">{introPanel}</div>}
              </div>
            </div>
          </motion.div>
        ) : null}
