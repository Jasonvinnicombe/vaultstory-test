      ...
      <CardContent className="space-y-8 p-8 sm:p-9 lg:p-11">
        <div className="flex flex-wrap items-start justify-between gap-5">
          <div className="section-stack max-w-2xl">
            <p className="text-sm uppercase tracking-[0.22em] text-white/60">Locked memory</p>
            <h1 className="text-balance font-display text-4xl lg:text-5xl">{props.title}</h1>
            <p className="text-sm leading-7 text-white/78">Recorded {getRecordedAgoLabel(props.createdAt)}</p>
          </div>
        ...
        <div className="grid gap-4 rounded-[30px] border border-white/12 bg-white/8 p-6 md:grid-cols-2">
          <div><p className="text-xs uppercase tracking-[0.2em] text-white/56">Created</p><p className="mt-2 text-sm leading-7 text-white/84">{formatDateTime(props.createdAt)}</p></div>
          <div><p className="text-xs uppercase tracking-[0.2em] text-white/56">Unlocks</p><p className="mt-2 text-sm leading-7 text-white/84">{props.unlockAt ? formatDateTime(props.unlockAt) : props.milestoneLabel ?? "Manual milestone"}</p></div>
        </div>

        <div className="space-y-4"><p className="inline-flex items-center gap-2 text-sm text-white/74"><CalendarClock className="h-4 w-4" />Countdown to reveal</p><CountdownTimer unlockAt={props.unlockAt} /></div>

        {props.canCompleteMilestone ? <div className="rounded-[30px] border border-white/12 bg-white/8 p-6"><p className="text-sm leading-7 text-white/82">This entry opens with a manual milestone. Mark it complete when the moment truly arrives.</p><div className="mt-4">{props.milestoneForm}</div></div> : null}
