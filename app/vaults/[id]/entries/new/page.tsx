...
export default async function NewEntryPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const { data: vault } = await supabase
    .from("vaults")
    .select("id, name, subject_name, subject_birthdate")
    .eq("id", id)
    .maybeSingle<Pick<VaultRow, "id" | "name" | "subject_name" | "subject_birthdate">>();
      ...
      <div className="section-stack">
        <Card className="overflow-hidden border-white/60 bg-card/88 shadow-[0_28px_84px_rgba(66,46,31,0.12)]">
          <CardContent className="p-8 sm:p-10">
            ...
            <h1 className="max-w-3xl text-balance font-display text-4xl text-foreground sm:text-5xl">Create a memory for {confirmedVault.subject_name ?? confirmedVault.name}.</h1>
            <p className="mt-4 max-w-2xl text-sm leading-7 text-muted-foreground sm:text-base">
              Capture the message, media, and timing for the exact moment this memory should return inside {confirmedVault.name}.
            </p>
        ...
        <EntryCreateForm vaultId={confirmedVault.id} subjectBirthdate={confirmedVault.subject_birthdate} currentPlan={profile?.membership_plan ?? "free"} />
      </div>
    </AppShell>
  );
}
